...
def dashboard():
    ...
    try:
        ...
        return render_template(
            "admin_dashboard.html", stats=stats, recent_products=recent_products
        )
    except Exception as e:
...
def products():
    ...
    try:
        ...
        return render_template(
            "admin_products.html", products=products, categories=categories
        )
    except Exception as e:
...
def api_create_product():
    ...
    try:
        ...
        if not price or price <= 0:
            return (
                jsonify({"success": False, "message": "El precio debe ser mayor a 0"}),
                400,
            )
        if not category:
            return (
                jsonify({"success": False, "message": "La categoría es requerida"}),
                400,
            )
        if stock is None or stock < 0:
            return (
                jsonify({"success": False, "message": "El stock debe ser 0 o mayor"}),
                400,
            )
        ...
        if "image" in request.files:
            ...
            if file and file.filename and allowed_file(file.filename):
                ...
                if save_uploaded_image(file, image_filename):
                    ...
                    return (
                        jsonify(
                            {"success": False, "message": "Error al guardar la imagen"}
                        ),
                        500,
                    )
...
def api_delete_product(product_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": f'Producto "{product_name}" eliminado'}
        )
